...
def welcome_screen(game_surface):
    ...
    selected_idx = 0
    press_text = "SPACE: Start   ESC/Q: Exit   ← →: Difficulty"
    ...
    while waiting:
        ...
        start_rect.center = (SCREEN_W // 2, SCREEN_H - 160)
        exit_rect.center  = (SCREEN_W // 2, SCREEN_H - 160 + btn_h + gap)

        mouse = pygame.mouse.get_pos()
        mx = int(mouse[0] * (SCREEN_W / window_w)) if window_w != SCREEN_W else mouse[0]
        my = int(mouse[1] * (SCREEN_H / window_h)) if window_h != SCREEN_H else mouse[1]

        # Start button
        pygame.draw.rect(game_surface,
                         (70, 200, 90) if start_rect.collidepoint(mx, my) else (50, 160, 70),
                         start_rect, border_radius=14)
        s_txt = font_small.render("Start", True, (255,255,255))
        game_surface.blit(s_txt, s_txt.get_rect(center=start_rect.center))

        # Exit button
        pygame.draw.rect(game_surface,
                         (200, 70, 70) if exit_rect.collidepoint(mx, my) else (160, 50, 50),
                         exit_rect, border_radius=14)
        e_txt = font_small.render("Exit", True, (255,255,255))
        game_surface.blit(e_txt, e_txt.get_rect(center=exit_rect.center))

        # Instruction text
        instr_surface = font_small.render(press_text, True, (200, 200, 200))
        instr_rect = instr_surface.get_rect(center=(SCREEN_W // 2, SCREEN_H - 40))
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                if event.key in (pygame.K_ESCAPE, pygame.K_q):
                    ...
                    return difficulties[selected_idx]  # start with selected difficulty
...
# --- FIXED: AI difficulty now follows the welcome selection ---
DIFFICULTY = PLAYER_DIFFICULTY.lower()
...
def main_game():
    global player_grid, ai_grid, player_score, ai_score
    global player_tet, player_next_tet, ai_tet, ai_plan
    global player_timer, ai_timer, diff_timer
    global last_power_time, power_active, power_owner, power_end_time
    global running, winner_side, winner_reason
    ...
    while running:
        ...
        # quick top-row full check each frame (handle both-full case)
        p_full = board_full(player_grid)
        a_full = board_full(ai_grid)
        if p_full or a_full:
            if p_full and not a_full:
                winner_side, winner_reason = 'AI', 'Player board full'
            elif a_full and not p_full:
                winner_side, winner_reason = 'Player', 'AI board full'
            else:
                # both full: decide by score (tie -> Player)
                if player_score >= ai_score:
                    winner_side, winner_reason = 'Player', 'both boards full (score tie/lead)'
                else:
                    winner_side, winner_reason = 'AI', 'both boards full (score lead)'
            running = False
            break
        ...
        if player_timer >= cur_player_speed:
            ...
            if not valid_position(player_tet, player_grid):
                ...
                # top-out check
                if not valid_position(player_tet, player_grid):
                    winner_side, winner_reason = 'AI', 'Player topped out'
                    running = False
                    break
        ...
        if ai_timer >= cur_ai_speed and running:
            ...
            if ai_plan['target_rot'] > 0:
                ...
                if ai_tet.x < ai_plan['target_x']:
                    ...
                    if not valid_position(ai_tet, ai_grid):
                        winner_side, winner_reason = 'Player', 'AI topped out'
                        running = False
                        break
...
# ----------------------
def game_over_screen(winner_side, winner_reason, player_score, ai_score):
    clock = pygame.time.Clock()

    # --- Detect full screen resolution
    info = pygame.display.Info()
    screen_w, screen_h = info.current_w, info.current_h
    screen = pygame.display.set_mode((screen_w, screen_h), pygame.FULLSCREEN)

    # Confetti setup if player won
    confetti = []
    if winner_side == "Player":
        for _ in range(120):
            ...
            confetti.append([x, y, color, random.randint(2, 6)])

    # --- Buttons
    button_w, button_h = 220, 60
    mainmenu_rect = pygame.Rect(screen_w//2 - button_w//2, screen_h//2 + 100, button_w, button_h)
    exit_rect     = pygame.Rect(screen_w//2 - button_w//2, screen_h//2 + 180, button_w, button_h)
    ...
    while running:
        ...
        text_surface = big_font.render(msg, True, color).convert_alpha()
        text_surface.set_alpha(alpha)
        text_rect = text_surface.get_rect(center=(screen_w//2, screen_h//2 - 80))
        screen.blit(text_surface, text_rect)

        # --- Scores & winner line
        winner_line = f"Winner: {winner_side}"
        if winner_reason:
            winner_line += f" ({winner_reason})"

        txt = font.render(winner_line, True, (230, 230, 230))
        ...
        # --- Confetti animation (Player only)
        if winner_side == "Player":
            for piece in confetti:
                pygame.draw.rect(screen, piece[2], (piece[0], piece[1], piece[3], piece[3]))
                piece[1] += piece[3]
                if piece[1] > screen_h:
                    piece[1] = random.randint(-50, -10)
                    piece[0] = random.randint(0, screen_w)

        # --- Dark fade (AI only)
        if winner_side == "AI":
            overlay = pygame.Surface((screen_w, screen_h))
            overlay.set_alpha(min(180, elapsed//5))
            overlay.fill((0, 0, 0))
            screen.blit(overlay, (0, 0))

        # --- Buttons
        mouse = pygame.mouse.get_pos()
        pygame.draw.rect(screen, (70, 130, 180) if mainmenu_rect.collidepoint(mouse) else (40, 100, 160),
                         mainmenu_rect, border_radius=12)
        pygame.draw.rect(screen, (200, 60, 60) if exit_rect.collidepoint(mouse) else (150, 30, 30),
                         exit_rect, border_radius=12)

        main_txt = font.render("Main Menu", True, (255, 255, 255))
        screen.blit(main_txt, main_txt.get_rect(center=mainmenu_rect.center))
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                if mainmenu_rect.collidepoint(event.pos):
                    return "menu"    # go back to Welcome screen
                if exit_rect.collidepoint(event.pos):
                    pygame.quit(); sys.exit()

# --------------- Main loop ---------------
while True:
    # Always show the welcome/difficulty menu first
    PLAYER_DIFFICULTY = welcome_screen(game_surface)
    PLAYER_BASE_SPEED = difficulty_to_player_speed(PLAYER_DIFFICULTY)

    # sync AI difficulty/mistake with the chosen difficulty
    DIFFICULTY = PLAYER_DIFFICULTY.lower()
    ai_mistake = AI_MISTAKE_CHANCE.get(DIFFICULTY, 0.12)
    ...
    # show animated end screen
    result = game_over_screen(winner_side, winner_reason, player_score, ai_score)

    if result == "menu":
        # loop again -> welcome screen -> new difficulty -> new game
        continue
    else:
        break